...
class MediaManager {
    ...
    async showPauseAd(ad) {
        ...
        // 小窗占容器约 25% 宽度，最小 120px，最大 260px
        const pipW = Math.min(260, Math.max(120, Math.round(containerW * 0.25)));
        const pipH = Math.round(pipW * 9 / 16); // 16:9 比例
    ...
    async showAd(ad) {
        ...
        // 暂停主视频（设标记防止触发暂停广告）
        this.skipNextPausePromo = true;
        this.player.pause();
        this.skipNextPausePromo = false;
    ...
    showOverlay(ad) {
        ...
        const containerW = container.offsetWidth || 600;
        const containerH = container.offsetHeight || 400;
        // 角标最大占容器 35% 宽、30% 高，最小 80px，最大 320px
