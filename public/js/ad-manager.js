...
class AdManager {
    ...
    createAdOverlay() {
        ...
        this.overlay.className = 'ad-overlay';
        ...
        // 广告视频容器
        this.adVideoWrap = document.createElement('div');
        this.adVideoWrap.style.cssText = `width:100%;height:100%;position:relative;`;
        this.overlay.appendChild(this.adVideoWrap);
        
        // 广告视频元素
        this.adVideo = document.createElement('video');
        this.adVideo.style.cssText = `width:100%;height:100%;object-fit:contain;`;
        this.adVideo.setAttribute('playsinline', '');
        this.adVideo.setAttribute('webkit-playsinline', '');
        this.adVideo.setAttribute('preload', 'auto');
        this.adVideoWrap.appendChild(this.adVideo);
        ...
        `;
        this.adBadge.textContent = '广告';
        topLeft.appendChild(this.adBadge);
        ...
        `;
        this.vipCloseBtn.textContent = 'VIP跳广告';
        ...
        // 广告视频事件
        this.adVideo.addEventListener('ended', () => this.endAd());
        this.adVideo.addEventListener('error', (e) => {
            console.error('广告视频加载失败:', e);
            this.endAd();
        });
    ...
    async loadAds() {
        try {
            ...
            this.adsLoaded = true;
            console.log('广告加载完成:', this.options.ads.length, '条');
    ...
    async getAdPlayInfo(adId) {
        const res = await fetch(`/api/ads/${adId}/play`);
        if (!res.ok) throw new Error('获取广告播放地址失败');
        const data = await res.json();
        if (!data.success || !data.mediaUrl) throw new Error(data.error || '广告播放地址不存在');
        return data;
    }
    ...
    /**
     * 从指定类型的广告中随机抽取一条
     * @param {string} type - 广告类型 (preroll/midroll/postroll/pause)
    ...
    async playPreRoll() {
        ...
        const ad = this.pickRandomAd('preroll');
        if (!ad) return false;
    ...
    checkMidRoll(currentTime) {
        ...
        // 检查是否有广告的 triggerTime 被当前时间命中（±1秒容差）
        for (const ad of midrollAds) {
            ...
            if (triggerTime > 0 && Math.abs(currentTime - triggerTime) < 1.5) {
                ...
                console.log('命中中贴片广告:', ad.name, '触发时间:', triggerTime + 's');
                this.showAd(ad);
                return true;
            }
        }
    ...
    async playPostRoll() {
        if (this.isAdPlaying) return false;
        const ad = this.pickRandomAd('postroll');
        if (!ad) return false;
        await this.showAd(ad);
        return true;
    }
    ...
    async showPauseAd(ad) {
        ...
        console.log('显示暂停广告:', ad.name);
        
        const mediaUrl = ad.media_url;
        if (!mediaUrl) { console.error('广告没有media_url'); return; }
        ...
        this.pauseOverlay.className = 'pause-ad-overlay';
        ...
        adVideoEl.src = mediaUrl;
        adVideoEl.style.cssText = `
            width:100%;height:100%;object-fit:contain;background:#000;display:block;
        `;
        adVideoEl.setAttribute('playsinline', '');
        adVideoEl.setAttribute('webkit-playsinline', '');
        adVideoEl.loop = false;
        this.pauseOverlay.appendChild(adVideoEl);
        ...
        `;
        adBadge.textContent = '广告';
        topLeft.appendChild(adBadge);
        ...
        skipBtn.onclick = () => this.endPauseAd();
        ...
        pipWrap.className = 'pause-ad-pip';
        ...
        pipPlayIcon.style.cssText = `
            ...
            pointer-events:none;
        `;
        pipPlayIcon.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="#fff"><path d="M8 5v14l11-7z"/></svg>`;
        ...
        // 倒计时（仅显示用，不自动结束，广告循环播放直到用户关闭或播放视频）
        let remaining = duration;
        this.timer = setInterval(() => {
            remaining--;
            if (remaining <= 0) {
                remaining = duration; // 重置倒计时（广告循环）
            }
            countdownEl.textContent = `${remaining}秒`;
        }, 1000);
        
        // 保存引用
        this._pauseAdVideo = adVideoEl;
    ...
    async showAd(ad) {
        ...
        console.log('显示广告:', ad.name, ad.type);
        ...
        if (playPromise) {
            playPromise.catch(() => {
                ...
                this.adVideo.play().catch(() => this.endAd());
            });
        }
    ...
    clearPauseAd() { return this.endPauseAd(); }
    ...
    showOverlay(ad) {
        ...
        const width = ad.width || 120;
        const height = ad.height || 60;
        img.style.cssText = `max-width:${width}px;max-height:${height}px;border-radius:6px;object-fit:cover;opacity:0.9;`;
        el.appendChild(img);
    ...
    _trackAdEvent(ad, event) {
        if (!ad) return;
        const ep = event === 'click' ? 'click' : event === 'skip' ? 'skip' : 'impression';
        fetch('/api/ads/' + ad.id + '/' + ep, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).catch(() => {});
    }
