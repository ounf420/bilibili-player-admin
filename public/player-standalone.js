/**
 * 影视中心播放器 Standalone v1.0
 * 一个JS文件，放到任何服务器都能用，广告系统走 dem.viesta.cn
 *
 * 使用方式：
 *   <div id="player-box" style="width:800px;height:500px"></div>
 *   <script src="/path/to/player.js"
 *     data-container="#player-box"
 *     data-url="https://xxx.com/video.mp4"
 *   ><\/script>
 *
 * 参数（data-xxx）：
 *   container   容器选择器（默认 #player-box）
 *   url         视频URL（mp4/m3u8/flv）
 *   type        强制格式 mp4/m3u8/flv（可选，自动识别）
 *   cover       封面图URL
 ...
 *   v           dem.viesta.cn系统视频ID（可选）
 *   ad          是否加载广告 true/false（默认true）
 */
(function(){
"use strict";
...
var OPT={
  container:SCRIPT.getAttribute("data-container")||"#player-box",
  url:SCRIPT.getAttribute("data-url")||"",
  type:SCRIPT.getAttribute("data-type")||"",
  cover:SCRIPT.getAttribute("data-cover")||"",
  ...
  logoPos:SCRIPT.getAttribute("data-logo-pos")||"tr",
  v:SCRIPT.getAttribute("data-v")||"",
  ad:SCRIPT.getAttribute("data-ad")!=="false"
};

/* ---------- 样式 ---------- */
var css=document.createElement("style");
css.textContent=[
  OPT.container+"{position:relative;width:100%;height:100%;background:#000;overflow:hidden}",
  ".__wm{position:absolute;z-index:100;pointer-events:none}",
  ".__wm.tl{top:12px;left:12px}.__wm.tr{top:12px;right:12px}",
  ".__wm.bl{bottom:60px;left:12px}.__wm.br{bottom:60px;right:12px}",
  ".__wm img{max-width:100px;max-height:32px;opacity:.7}",
  ".__wm span{color:rgba(255,255,255,.5);font-size:12px;text-shadow:0 1px 3px rgba(0,0,0,.5);user-select:none}"
].join("\n");
...
/* ---------- 广告引擎（绝对路径版）---------- */
/** AD_BASE: 广告API地址 */
var AD_BASE='https://dem.viesta.cn';
/**
 * DPlayer 推广管理系统 v5
 * 画中画暂停推广 + 全推广随机抽取
 */
...
class MediaManager {
    ...
    async loadCampaigns() {
        try {
            const [adsRes, settingsRes] = await Promise.all([
                fetch(AD_BASE+'/api/campaigns'),
                fetch(AD_BASE+'/api/settings')
            ]);
    ...
    async getAdPlayInfo(promoId) {
        const res = await fetch(AD_BASE+`/api/campaigns/${promoId}/play`);
    ...
    hasPreRoll() {
        return this.options.ads.some(ad => ad.type === 'preroll' && ad.enabled);
    }
    ...
    trackEvent(event) {
        ...
        fetch(AD_BASE+'/api/campaigns/track', {
            method: 'POST',
    ...
    _trackEvent(ad, event) {
        ...
        fetch(AD_BASE+'/api/campaigns/' + ad.id + '/' + ep, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).catch(() => {});
    }
...
/* ---------- 主逻辑 ---------- */
var AD_ORIGIN="https://dem.viesta.cn";

async function main(){
  // 加载播放器依赖
  await Promise.all([
    loadJS("https://cdn.jsdelivr.net/npm/dplayer@1.27.1/dist/DPlayer.min.js"),
    loadJS("https://cdn.jsdelivr.net/npm/hls.js@1.5.17/dist/hls.min.js"),
    loadJS("https://cdn.jsdelivr.net/npm/flv.js@1.6.2/dist/flv.min.js")
  ]);

  // 获取容器
  var box=document.querySelector(OPT.container);
  if(!box){console.error("[Player] 容器不存在:",OPT.container);return}

  // 解析视频
  var video;
  if(OPT.url){
    video={url:OPT.url,type:OPT.type||detectType(OPT.url),cover:OPT.cover||"",title:""};
  }else if(OPT.v){
    try{var r=await fetch(AD_ORIGIN+"/api/videos/"+OPT.v);video=await r.json()}catch(e){console.error("[Player]",e)}
  }
  ...
  if(video.type==="m3u8"&&typeof Hls!=="undefined"&&Hls.isSupported()){
    vc.type="customHls";vc.customType={customHls:function(vEl){
      var h=new Hls();h.loadSource(vEl.src);h.attachMedia(vEl);
      h.on(Hls.Events.MANIFEST_PARSED,function(){vEl.play()});
      h.on(Hls.Events.ERROR,function(_,d){if(d.fatal)h.destroy()});
    }};
  }else if(video.type==="flv"&&typeof flvjs!=="undefined"&&flvjs.isSupported()){
    vc.type="customFlv";vc.customType={customFlv:function(vEl){
      var f=flvjs.createPlayer({type:"flv",url:vEl.src});f.attachMediaElement(vEl);f.load();f.play();
    }};
  }
  ...
  var dp=new DPlayer({
    ...
    theme:OPT.theme,
    lang:"zh-cn",
    screenshot:true,hotkey:true,preload:"auto",volume:.7,mutex:true,
    video:vc
  });
  ...
  // 水印
  if(OPT.logo||OPT.logoText){
    var wm=document.createElement("div");
    wm.className="__wm "+OPT.logoPos;
    wm.innerHTML=OPT.logo?'<img src="'+OPT.logo+'">':'<span>'+OPT.logoText+'</span>';
    box.appendChild(wm);
  }

  console.log("[Player] OK"+(OPT.ad?" (含广告)":" (无广告)"));
}
